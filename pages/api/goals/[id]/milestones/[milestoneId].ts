...
async function updateMilestone(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Update milestone
    const milestone = await service.updateMilestone(id, milestoneId, updates, user_id);
...
async function deleteMilestone(req: NextApiRequest, res: NextApiResponse) {
  // Verify authentication
  const authResult = await verifyAuth(req);
  if ('error' in authResult) {
    return res.status(authResult.error.status).json({ error: authResult.error.message });
  ...
  try {
    ...
    // Delete milestone
    await service.deleteMilestone(id, milestoneId, user_id);
