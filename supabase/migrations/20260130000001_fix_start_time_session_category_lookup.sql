-- Migration: Fix start_time_session to properly look up category hourly_rate
-- Date: 2026-01-30
-- Issue: start_time_session checks created_by when looking up category rate, which may fail
-- Solution: Remove created_by check - categories are shared or user-specific based on RLS policies
...
BEGIN
    ...
    -- Remove created_by check - RLS policies handle access control
    IF p_hourly_rate_usd IS NULL AND task_category_id IS NOT NULL THEN
        ...
        -- Only use category rate if it's not NULL
        IF category_rate IS NOT NULL THEN
...
'Starts a new time session for a task. Accepts p_user_id parameter to support service role key calls where auth.uid() returns NULL. Falls back to auth.uid() if p_user_id is not provided. Looks up category hourly_rate without created_by check (RLS handles access).';
