...
ADD COLUMN IF NOT EXISTS goal_id UUID,  -- Foreign key to goals table (to be added when goals table exists)
...
DROP FUNCTION IF EXISTS get_active_carousel_images(UUID);

CREATE FUNCTION get_active_carousel_images(p_user_id UUID DEFAULT NULL)
RETURNS TABLE (
    id UUID,
    file_path TEXT,
    title VARCHAR(200),
